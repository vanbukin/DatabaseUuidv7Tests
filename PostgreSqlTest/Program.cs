...
public static class Program
{
    private const string ConnectionString =
        "Database=dotnet;Host=localhost;Password=postgres;Port=5432;Username=postgres";

    private const int NumberOfRuns = 15;
    private const int RunSize = 1_000_000;
    private const int TransactionChunkSize = 100_000;
    private const int SingleInsertChunkSize = 25_000;
    ...
    static async Task Main(string[] args)
    {
        ...
        for (int i = 1; i <= NumberOfRuns; i++)
        {
            ...
            var uploadedItems = await UploadUuidsAsync(uuids, TransactionChunkSize, SingleInsertChunkSize, alreadyInserted);
            swUpload.Stop();
